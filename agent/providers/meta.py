...
class ProveedorMeta(ProveedorWhatsApp):
    ...
    def __init__(self):
        ...
        self.verify_token = os.getenv("META_VERIFY_TOKEN", "etudehorizon-verify")
    ...
    async def parsear_webhook(self, request: Request) -> list[MensajeEntrante]:
        """Parsea el payload anidado de Meta Cloud API (texto + documentos/imágenes)."""
        ...
        for entry in body.get("entry", []):
            for change in entry.get("changes", []):
                ...
                for msg in value.get("messages", []):
                    ...
                    # Documentos / imágenes / audio / video: capturamos el media id + tipo
                    elif tipo in ("document", "image", "audio", "video"):
                        media = msg.get(tipo, {})
                        media_id = media.get("id", "")
                        mime = media.get("mime_type", tipo)
                        mensajes.append(MensajeEntrante(
                            telefono=telefono,
                            texto=media.get("caption", ""),
                            mensaje_id=mensaje_id,
                            es_propio=False,
                            media=[{"url": media_id, "tipo": mime}],
                        ))
        return mensajes
